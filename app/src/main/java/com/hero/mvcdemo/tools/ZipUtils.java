 ...
 * @author ninemax
 * 参阅
 * https://blog.csdn.net/bjstyle/article/details/72637119
 */
public class ZipUtils {
     ...
     *
     * @param src    要压缩的文件
     * @param dest   压缩文件存放路径   只能是空路径   不能是文件的路径
     * @param passwd 压缩使用的密码
     * @return 最终的压缩文件存放的绝对路径, 如果为null则说明压缩失败.
     */
...
//            dest = buildDestinationZipFilePath(srcFile, dest);   不需要他的处理
            ...
            String parentDir = dest.substring(0, dest.lastIndexOf(File.separator));
            File dir = new File(parentDir);
            if (dir.exists()) {
                if (!dir.isDirectory()) {
                    dir.delete();   //老版本(2.5.0以前)中存在创建的IBLLogs_A_ZIP不是一个文件夹的情况
                    dir.mkdirs();
                }
            } else {
                dir.mkdirs();
